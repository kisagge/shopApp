# Vercel 배포

프론트와 백이 한 프로젝트라 **별도 백엔드 서버가 필요 없다.** API 라우트 24개,
서버 렌더링 화면, Better Auth 핸들러, Prisma 쿼리, 토스 승인 호출이 전부
Vercel 서버리스 함수로 돈다.

다만 **Postgres 와 오브젝트 스토리지는 따로 마련해야 한다.** 지금은 로컬 Docker 다.

---

## 1. 관리형 Postgres

Neon · Supabase · Vercel Postgres 중 아무거나.

### 리전은 DB 와 함수를 **같은 곳**에 둔다

**여기 적혀 있던 말이 따를 수 없는 지시였다.** 예전에는 "리전은 `icn1`(서울)에
맞춘다" 고 적혀 있었는데, **Neon 에는 서울 리전이 없다**(AWS 8개 중 아시아는
`ap-southeast-1` 싱가포르와 `ap-southeast-2` 시드니뿐이다). 그래서 DB 는
싱가포르에 놓이고 `vercel.json` 은 서울을 가리킨 채로 굳었다.

값으로 재면 이렇다. 기기에서 같은 조건으로 번갈아 재서 서로 뺄 수 있게 했다.

| 무엇 | 값 |
|---|---|
| 아무 일도 안 하는 창구(`/api/health`) | 169ms |
| DB 질의 **하나**를 하는 창구 | 492ms |
| 서버가 스스로 잰 그 질의 시간(`db;dur`) | **141ms** (여섯 번 재서 141·141·142·142·141) |

질의 내용과 무관하게 값이 고정이면 **질의가 느린 것이 아니라 거리가 먼 것**이다.
그리고 주문 하나를 만드는 데 왕복을 10~15번 쓴다 — 그것이 곧 1.5~2초다.

Neon 은 **기존 프로젝트의 리전을 바꿀 수 없다.** 그래서 붙이는 방법은 함수를
DB 쪽으로 옮기는 것뿐이고, `vercel.json` 의 `regions` 를 `sin1`(싱가포르)로 둔다.

한국 사용자 기준으로 요청 하나의 왕복은 그만큼 늘어난다(서울↔싱가포르). 그래도
**요청 하나에 붙는 거리 한 번과, 그 요청 안에서 열 번 넘게 오가는 거리를 맞바꾸는
것**이라 이쪽이 크게 이긴다. 정적 파일은 원래 에지에서 나가므로 영향이 없다.

DB 를 옮길 거라면 함수 리전도 함께 옮긴다. **둘이 어긋난 채로 두는 것이 가장
나쁘다** — 아무 데도 가깝지 않으면서 양쪽 거리를 다 치른다.

**주소가 두 개 필요하다.**

| 환경변수 | 무엇 | 왜 |
|---|---|---|
| `DATABASE_URL` | 풀러를 거친 주소 | 서버리스는 함수 인스턴스마다 커넥션을 연다. 직접 붙으면 금방 고갈된다 |
| `DIRECT_DATABASE_URL` | 풀러를 거치지 않은 주소 | 트랜잭션 모드 풀러는 DDL 과 어드바이저리 락을 제대로 다루지 못해 마이그레이션이 중간에 멈춘다 |

Neon 은 `-pooler` 가 붙은 호스트가 풀링 주소이고, Supabase 는 6543 포트가 풀러,
5432 가 직결이다.

**Neon 을 Vercel 마켓플레이스로 붙였다면 `DIRECT_DATABASE_URL` 을 따로 넣지 않아도 된다.**
Neon 이 직결 주소를 `DATABASE_URL_UNPOOLED` 로 자동 주입하고, `prisma.config.ts` 가
그 이름도 함께 읽는다. 비밀번호가 든 URL 을 손으로 옮겨 적지 않는 편이 안전하다.

## 2. 오브젝트 스토리지

**S3 호환이어야 한다.** 어댑터가 `@aws-sdk/client-s3` 로 붙는다 —
Vercel Blob 은 S3 API 가 아니라 쓸 수 없다.

**`products/` 접두사를 익명 읽기로 열어 둔다.** 이미지는 브라우저가 직접
가져간다. 우리 함수로 프록시하면 요청마다 함수가 깨어나고 서버리스 실행 시간을
이미지 전송에 쓴다.

### Cloudflare R2 (권장)

이미지 전송 요금이 없어 이런 용도에 맞는다.

1. R2 → Create bucket
2. R2 → Manage API Tokens → Create → **Object Read & Write**
   → Access Key ID 와 Secret Access Key 를 준다
3. 버킷 → Settings → **Public Development URL** 활성화
   (또는 커스텀 도메인 연결)

| 변수 | 값 |
|---|---|
| `S3_ENDPOINT` | `https://<계정ID>.r2.cloudflarestorage.com` |
| `S3_REGION` | `auto` |
| `S3_BUCKET` | 버킷 이름 |
| `S3_ACCESS_KEY_ID` / `S3_SECRET_ACCESS_KEY` | 2번의 토큰 |
| `S3_PUBLIC_BASE_URL` | 3번의 공개 주소 (`https://pub-xxxx.r2.dev`) |
| `S3_FORCE_PATH_STYLE` | `true` |

R2 는 S3 의 버킷 정책 API 를 지원하지 않으므로 `pnpm storage:setup` 의 정책
단계는 건너뛴다(경고만 남기고 멈추지 않는다). 공개 설정은 3번이 대신한다.

### AWS S3

| 변수 | 값 |
|---|---|
| `S3_ENDPOINT` | 비워 둔다 (AWS 기본 엔드포인트) |
| `S3_REGION` | `ap-northeast-2` |
| `S3_PUBLIC_BASE_URL` | `https://<버킷>.s3.ap-northeast-2.amazonaws.com` |
| `S3_FORCE_PATH_STYLE` | `false` |

IAM 사용자에게 `s3:PutObject`, `s3:DeleteObject` 를 준다. 공개 읽기는
`pnpm storage:setup` 이 넣는 버킷 정책이 처리한다(퍼블릭 액세스 차단은 꺼야 한다).

### 배포 후 이미지 채우기

시드된 이미지 URL 은 로컬 MinIO 를 가리키므로 배포본에서는 깨진다.
운영 스토리지를 가리켜 다시 넣는다.

```bash
# 기존 행을 지운 뒤 (시드는 이미지가 있으면 건너뛴다)
psql "<운영 DB 주소>" -c 'delete from product_images;'

DATABASE_URL="<운영 DB 주소>" \
S3_ENDPOINT="..." S3_BUCKET="..." S3_REGION="..." \
S3_ACCESS_KEY_ID="..." S3_SECRET_ACCESS_KEY="..." \
S3_PUBLIC_BASE_URL="..." \
  pnpm storage:seed
```

## 3. 환경변수

Vercel 프로젝트 설정 → Environment Variables. **시크릿 세 개는 직접 만들어 넣는다.**

```bash
openssl rand -base64 32   # BETTER_AUTH_SECRET
openssl rand -base64 32   # EVENT_IP_SALT
openssl rand -base64 32   # CRON_SECRET
```

| 변수 | 값 | 비워 두면 |
|---|---|---|
| `DATABASE_URL` | 풀링 주소 | 앱이 뜨지 않는다 |
| `DIRECT_DATABASE_URL` | 직결 주소 (Neon 통합이면 불필요) | 마이그레이션이 풀러를 타서 멈출 수 있다 |
| `BETTER_AUTH_SECRET` | 랜덤 32바이트 | 세션 서명이 안 된다 |
| `BETTER_AUTH_URL` | `https://<운영 도메인>` | 프리뷰는 `VERCEL_URL` 로 자동 해결. **운영에는 넣는다** |
| `NEXT_PUBLIC_APP_URL` | `https://<운영 도메인>` | Capacitor 셸이 붙을 주소 |
| `EVENT_IP_SALT` | 랜덤 32바이트 | 이벤트 IP 해시가 고정 솔트로 약해진다 |
| `CRON_SECRET` | 랜덤 32바이트 | **배치 라우트가 503 으로 거절한다** (열어 두지 않는다) |
| `S3_*` | 스토리지 값 6개 | 이미지 업로드가 503 으로 거절한다 |
| `TOSS_SECRET_KEY` / `NEXT_PUBLIC_TOSS_CLIENT_KEY` | 토스 키 | **결제 화면이 막힌다.** 아래 "결제 키 없이 배포할 때" 참고 |

### 결제 키 없이 배포할 때

**이 표의 예전 설명이 틀렸다.** "Mock 게이트웨이로 돈다 (배포는 성공한다)"
라고 적혀 있었는데, 실제로는 배포는 뜨고 **주문 확정에서 500** 이 났다.
브라우저는 클라이언트 키가 없는 것을 보고 "결제창 없이 진행" 으로 갔고,
서버는 시크릿 키가 없는 것을 보고 "프로덕션에서 Mock 은 못 쓴다" 며 던졌다.
두 판단이 서로를 몰랐다. 주문 `20260910-7063897` 이 그렇게 입금대기로 남았다.

지금은 판단을 서버가 한 번만 한다. 키가 없으면 **결제 화면 자체가 막힌다** —
주문이 만들어지기 전에 멈추므로 결제 없는 주문이 생기지 않는다.

주문 흐름을 끝까지 보여 주는 데모 배포라면 두 값을 함께 넣는다.

| 값 | 뜻 |
|---|---|
| `PAYMENT_GATEWAY=mock` | 결제창 없이 승인을 흉내낸다 |
| `DEMO_PAYMENT=1` | **이 배포는 돈이 오가지 않는 데모**라고 선언한다 |

**둘 다 있어야 한다.** `PAYMENT_GATEWAY=mock` 하나만으로는 운영 배포에서
절대 켜지지 않는다 — 실수로 켜지면 결제 없이 주문이 확정되기 때문이다.
진짜 키가 함께 있으면 어느 쪽이 잘못됐는지 알 수 없으므로 그때도 던진다.

실제 결제를 붙일 거라면 두 키를 **함께** 넣는다. 시크릿만 넣으면 결제창을
띄울 수 없어 역시 화면이 막힌다.

`BETTER_AUTH_URL` 은 **비워 둬도 된다.** 운영에서는 Vercel 이 주는
`VERCEL_PROJECT_PRODUCTION_URL`(안정 도메인)을, 프리뷰에서는 `VERCEL_URL`
(그 배포 주소)을 쓴다.

넣을 거라면 **Production 환경에만** 넣는다. Preview 에 운영 도메인을 넣으면
프리뷰의 Origin 과 어긋나 로그인이 막힌다.

> 운영에서 `VERCEL_URL` 을 쓰면 안 된다. 배포마다 바뀌는 주소라 사용자가
> 실제로 접속하는 안정 도메인과 달라 `Invalid origin` 이 난다. 실제로 겪었다.

## 4. 배포

GitHub 저장소를 Vercel 에 연결하면 `vercel.json` 을 그대로 읽는다.

**Root Directory 는 `apps/web` 이다.** Vercel 은 그 디렉터리의 `package.json` 에서
`next` 를 찾아 프레임워크를 감지한다. 저장소 루트로 두면
"No Next.js version detected" 로 죽는다.

그래서 **`vercel.json` 도 `apps/web/` 에 있다.** Vercel 은 Root Directory 기준으로
이 파일을 읽는다.

```
installCommand  pnpm install --frozen-lockfile
buildCommand    check-db-url && prisma migrate deploy && turbo run build --filter=@shop/web
```

`outputDirectory` 는 `.next` 다 — Root Directory 기준으로 풀린다.
`apps/web/.next` 로 적으면 `apps/web/apps/web/.next` 를 찾다가 실패한다.

**명시해 두는 이유가 있다.** 가져오기 화면에서 잡힌 값이 대시보드에 남아 있으면
그것이 적용되는데, `vercel.json` 이 대시보드 설정보다 우선하므로 여기서 못박아
둬야 어느 프로젝트에서 배포하든 같은 결과가 나온다.

빌드 명령의 세 단계는 모두 `pnpm --filter` 를 거친다. pnpm 이 워크스페이스를
스스로 찾으므로 **실행 위치에 매이지 않는다.** 저장소 루트 기준 상대 경로를
쓰면 Root Directory 설정에 따라 파일을 못 찾는다.

빌드 첫 단계가 DB 주소를 확인한다. 비어 있으면 어느 변수를 어디에 넣어야 하는지
적어 주고 멈춘다 — Prisma 는 "Connection url is empty" 만 말해서 로그만으로는
원인을 좁힐 수 없다.

**마이그레이션이 빌드 안에서 돈다.** 실패하면 배포도 안 되므로, 스키마와 코드가
어긋난 채로 뜨는 일이 없다. 첫 배포에서 `0_init` 이 전체 스키마를 만든다.

**시드는 배포에 안 딸려 온다.** 마이그레이션만 빌드 안에서 돈다. 그래서
`seed.ts` 에 상품을 더해도 **배포본의 매대는 그대로다** — 로컬에서만 늘어난다.
실제로 아우터를 스물여섯으로 올려 "더 보기" 를 볼 수 있게 해 놓고도, 배포본은
열셋 그대로여서 그 화면이 안 나왔다.

빌드 후 시드가 필요하면 로컬에서 운영 DB 를 가리켜 돌린다:

```bash
SEED_REMOTE=yes DATABASE_URL="<운영 풀링 주소>" pnpm db:seed
```

**`SEED_REMOTE=yes` 가 없으면 거절한다.** 셸에 운영 주소를 남겨 둔 채 다른
명령을 돌리는 실수를 막으려고 일부러 둔 문지기다(`seed-target.ts`). 예전에
여기 적혀 있던 명령에는 이 플래그가 빠져 있어서, 그대로 따라 하면 멈췄다.

시드가 건드리는 것은 상품만이 아니다 — 배너·기획전·공지·FAQ 를 upsert 하고,
**리뷰가 없는 상품에는 리뷰와 그 리뷰가 딸린 주문을 만든다.** 데모용 자료라
그래도 되는 자리지만, 운영 DB 에 가짜 주문이 는다는 것은 알고 돌려야 한다.

## 5. 크론

`apps/web/vercel.json` 에 여섯 개가 정의돼 있다. **크론 표현식은 UTC 로 읽힌다** —
아래 시각은 거기에 9시간을 더한 값이다.

| 경로 | 주기 | 하는 일 |
|---|---|---|
| `/api/cron/expire-points` | 매일 KST 02:00 | 기한이 지난 적립금을 소멸시킴 |
| `/api/cron/reconcile-points` | 매일 KST 03:00 | 포인트 잔액을 원장에 맞춤 |
| `/api/cron/rollup-events` | 매일 KST 04:00 | 어제치 이벤트를 일별 집계로 접고, 90일 지난 원본을 지움 |
| `/api/cron/auto-confirm` | 매일 KST 05:00 | 배송완료 8일 지난 주문을 구매확정하고 적립을 지급 |
| `/api/cron/release-holds` | 매일 KST 06:00 | 결제까지 안 간 주문이 물고 있던 재고를 푼다 |
| `/api/cron/expiry-notices` | 매일 KST 10:00 | 한 주 안에 사라질 쿠폰·적립금을 손님에게 한 번 알린다 |
| `/api/cron/settlements` | 매달 2일 KST 05:00 | 앞 달 정산 확정 |

**정산이 1일이 아니라 2일인 이유.** 표현식 `0 20 1 * *` 은 UTC 1일 20:00 이고,
그것이 KST 로는 2일 05:00 이다. KST 1일 05:00 에 돌리려면 크론이 "전달 마지막
날"을 가리켜야 하는데 그런 표현이 없다. 앞 달이 확실히 끝난 뒤에 도는 편이
정산에는 맞기도 하다.

예전에는 이 표에 "두 개가 정의돼 있다" 며 넷만 적혀 있었고 정산은 1일이라고
적혀 있었다. 셋 다 틀렸다. 이제 `cron-coverage` 검사가 `vercel.json` 에서
시각을 계산해 이 표와 맞는지 본다 — 손으로 적은 표는 조용히 어긋난다.

## 토스 결제 테스트

키는 [토스 개발자센터](https://developers.tosspayments.com) 에서 직접 발급받는다.
문서용 테스트 키는 가입 없이 쓸 수 있고, 가입하면 **개발 연동 체험 상점**이
생겨 가상계좌 입금 시뮬레이션과 웹훅 설정을 쓸 수 있다.

테스트할 때 알아 둘 것:

- **국내 테스트 카드번호는 없다.** 테스트 환경에서도 실제 카드 정보를 입력해야
  하고, 대신 실제 청구는 일어나지 않는다. 이건 각자 자기 카드로 확인할 일이다.
- **가상계좌 입금은 개발자센터에서 시뮬레이션한다.** 실제로 송금할 필요가 없다.
  시뮬레이션을 돌리면 우리 웹훅으로 호출이 오고, 주문이 결제 대기 → 결제 완료로
  넘어가야 정상이다.
- **간편결제는 테스트로 확인할 수 없다.** 카카오페이는 실 계약이 있어야 하고
  네이버페이는 테스트에서 제대로 동작하지 않는다. 그래서 실제 클라이언트 키가
  설정된 환경에서는 결제 수단 목록에서 아예 감춘다 — 남겨 두면 간편결제만
  조용히 Mock 으로 빠져 결제창도 없이 주문이 완료된다.
- 테스트 모드의 영수증 링크에는 실제 영수증 데이터가 없다.

키 없이도 주문 흐름 전체를 볼 수 있다. Mock 게이트웨이가 결제창 없이 승인하고,
서버 쪽 흐름(금액 검증·멱등·상태 전이)은 실제와 같다.

## 토스 웹훅

가상계좌는 결제창을 닫는 순간 끝나지 않는다. 며칠 뒤에 입금할 수도 있고,
그 입금을 알려 주는 것이 웹훅이다. 토스 개발자센터 > 웹훅 에서 등록한다.

```
https://<도메인>/api/webhooks/toss
```

**본문은 믿지 않는다.** 이 주소는 공개돼 있어 누구나 아무 내용이나 보낼 수
있으므로, 본문에서는 `paymentKey` 만 꺼내고 상태·금액은 토스 API 에 다시
물어서 확인한다. 그래서 별도의 서명 검증 설정이 필요 없다.

확인:

```bash
curl -X POST https://<도메인>/api/webhooks/toss \
  -H 'content-type: application/json' \
  -d '{"data":{"paymentKey":"forged","status":"DONE"}}'
# {"received":true,"applied":false,"reason":"우리 결제가 아닙니다"} 여야 정상
```

Vercel 은 `CRON_SECRET` 이 설정돼 있으면 `Authorization: Bearer` 로 보낸다.
네 라우트 모두 그 값을 확인하고, **시크릿이 없으면 열어 두지 않고 503 으로 막는다.**

플랜에 따라 크론 개수와 주기에 제한이 있으니 확인할 것.

### 배치를 손으로 돌려 보기

운영에 걸기 전에 **일회용 DB 에 대고** 한 번 돌려 보는 것이 좋다. 개발 DB 를
쓰면 주문이 구매확정되고 포인트가 지급돼 되돌릴 수 없다.

```bash
docker exec shop-postgres psql -U shop -d postgres -c 'create database plain_batch_check;'
export DATABASE_URL=postgresql://shop:shop@localhost:5432/plain_batch_check
pnpm --filter @shop/db exec prisma migrate deploy && pnpm db:seed
# 그다음 배치 함수를 직접 부른다(창구는 CRON_SECRET 을 요구한다).
#   releaseAbandonedHolds(now) · autoConfirmDelivered(now) · expirePoints(now)
#   reconcilePoints(actor, { fix: true }) · runEventRollup(now)
#   closeSettlements(actor, previousYearMonth(now))
# actor 는 크론이 쓰는 것과 같다 — { id: 'system:cron', role: 'SUPER_ADMIN', merchantId: null }
docker exec shop-postgres psql -U shop -d postgres -c 'drop database plain_batch_check;'
```

**시드만 넣고 돌리면 대부분 0건이다.** 오래된 주문도, 만료될 포인트도, 접을
이벤트도 없기 때문이다. 그건 배선만 확인한 것이지 일을 하는지 본 것이 아니다.
조건을 만들어야 한다 — 예를 들어 주문 하나를 `status='DELIVERED'`,
`deliveredAt = now() - interval '9 days'`, `rewardPoints = 2890` 으로 두고
`autoConfirmDelivered` 를 돌리면 확정·적립·원장·만료일까지 한 번에 볼 수 있다.

**정산 행을 직접 조회할 때 기간 경계를 조심한다.** `periodStart` 는 KST
자정이라 2026년 8월분은 `2026-07-31 15:00 UTC` 다. `>= '2026-08-01'` 로 찾으면
행이 없는 것처럼 보이고, 배치가 "이미 확정돼 건너뛴다" 고 말하는데 행은 안
보이는 상태가 된다 — 실제로 그렇게 한참 헤맸다.

## 6. 배포 후 확인

```bash
curl https://<도메인>/api/health
curl -o /dev/null -w '%{http_code}\n' https://<도메인>/api/cron/settlements   # 401 이어야 정상
```

- 로그인 → 마이페이지가 열리는가 (Origin 설정 확인)
- 상품 이미지가 보이는가 (스토리지 공개 정책 확인)
- 어드민 → 배너에서 문구를 바꿔 홈에 반영되는가

---

## 로컬 개발

```bash
pnpm db:up          # Postgres + MinIO
pnpm db:deploy      # 마이그레이션 적용
pnpm storage:setup  # 버킷 생성 + 공개 정책
pnpm db:seed
pnpm storage:seed   # 상품 플레이스홀더 이미지
pnpm banners:seed
pnpm dev
```

스키마를 바꿀 때:

```bash
pnpm db:migrate     # 마이그레이션 생성 + 적용 + 클라이언트 재생성
```

`pnpm db:push` 는 이력을 남기지 않으므로 **버릴 실험에만** 쓴다.
그리고 스키마를 바꾼 뒤에는 **dev 서버를 재시작해야 한다** — 실행 중인 서버는
예전 Prisma 클라이언트를 물고 있다.
