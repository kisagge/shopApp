...
const coat: CartLine = {
  ...
  listPrice: won(413_000), discountPercent: 30, quantity: 1,
};
const knit: CartLine = {
  variantId: 'v-knit-l', productName: '램스울 크루넥 니트',
  listPrice: won(129_000), discountPercent: 0, quantity: 1,
};
...
describe('calculateCart — 시안의 장바구니 화면과 같은 금액이 나와야 한다', () => {
  ...
  it('상품 할인 123,900', () => expect(totals.productDiscount).toBe(123_900));
  it('할인 후 418,100', () => expect(totals.merchandiseTotal).toBe(418_100));
  it('5만원 넘으니 무료배송', () => expect(totals.shipping.fee).toBe(0));
  it('결제 예정 418,100', () => expect(totals.payable).toBe(418_100));
  it('적립 1% = 4,181P', () => expect(totals.rewardPoints).toBe(4_181));
});
...
describe('쿠폰', () => {
  ...
  it('정액 쿠폰이 할인 후 금액에서 차감된다', () => {
    ...
    expect(t.payable).toBe(408_100);
  });

  it('최소 주문금액에 못 미치면 적용되지 않는다', () => {
    const t = calculateCart({
      lines: [{ ...knit, listPrice: won(20_000) }],
      coupon: amountCoupon,
    });
...
describe('포인트', () => {
  ...
  it('포인트로 결제한 몫은 적립 대상에서 빠진다', () => {
    ...
    expect(withPoints.rewardPoints).toBe(4_151); // (418,100 - 3,000)의 1%
  });
});
...
describe('할인 적용 순서', () => {
  it('무료배송 판정은 쿠폰·포인트 차감 전 금액으로 한다', () => {
    ...
    const t = calculateCart({
      lines: [{ ...knit, listPrice: won(55_000) }],
...
describe('수량', () => {
  it('여러 개면 정가·할인이 함께 곱해진다', () => {
    ...
    expect(t.productDiscount).toBe(371_700);
  });
