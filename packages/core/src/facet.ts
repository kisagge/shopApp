 ...
 * IN 절을 부풀릴 수 있다.
 *
 * **매대가 내놓는 값보다 넉넉히 위여야 한다.** 10 이던 시절, 상품이 서른넷이
 * 되면서 검색 화면 한 곳에 사이즈 칩이 열둘 떴다. 그것을 전부 누르면 계약이
 * 열 개를 넘는다며 **고른 것을 통째로 버렸고**, 화면은 조건 없는 목록과
 * 빈 체크박스로 돌아왔다. 누른 사람 입장에서는 아무 일도 일어나지 않는다.
 *
 * 지금 매대의 값 종류는 사이즈 17 · 색상 14 다. packages/db 의 검사가 이
 * 값이 그것들보다 위인지 계속 본다 — 매대가 자라면 그 검사가 먼저 진다.
 */
