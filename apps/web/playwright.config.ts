...
export default defineConfig({
  ...
  projects: [
    { name: 'setup', testMatch: /auth\.setup\.ts/ },
    {
      ...
      testMatch:
        /(auth|account-settings|shopping|a11y|a11y-public|pwa|i18n|recently-viewed|support|recommendations|search-suggest|collections|security-headers|brand-and-filters|compare|layout|image-priority|bundle-budget|web-vitals|sitemap|structured-data|layout-i18n|dark-contrast|theme-flip|theme-choice)\.spec\.ts/,
    },
    {
      name: 'customer',
      use: { ...devices['Desktop Chrome'], storageState: STATE_FILE.customer },
      testMatch:
        /(customer|review-helpful|notifications|order-idempotency|a11y-account|layout-customer|layout-customer-locales|prefetch-budget|nav-feedback|order-lifecycle|deposit-webhook|checkout-total|payment-callback|stock-race|double-spend|order-search|low-stock-alert|partial-cancel|partial-return|merchant-return|wishlist-restock|exchange|purchase-confirm|address-edit|coupon-download|review-edit|cart-reorder)\.spec\.ts/,
      dependencies: ['setup'],
    },
    {
      name: 'admin',
      ...
      testMatch:
        /(admin|slug-history|a11y-admin|layout-admin|layout-admin-locales|separation-of-duties|admin-drawer|dark-contrast-admin|rich-editor|shipping-policy-admin|product-review-notice)\.spec\.ts/,
      dependencies: ['setup'],
    },
    {
      name: 'merchant',
