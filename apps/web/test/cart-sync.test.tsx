...
const useSession = vi.hoisted(() => vi.fn<() => any>());
vi.mock('@shop/auth/client', () => ({ authClient: { useSession } }));
...
function signedIn(id: string | null) {
  useSession.mockReturnValue({ data: id ? { user: { id } } : null, isPending: false });
}
...
describe('장바구니 서버 맞추기', () => {
  ...
  it('로그인하면 한 번만 병합한다', async () => {
    signedIn('u1');
    const view = render(<CartSync />);
    await act(async () => {});
    view.rerender(<CartSync />);
    await act(async () => {});
