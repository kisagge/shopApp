 ...
 * 묶음에 들어가면 **주문을 보러 온 사람도 그 무게를 치른다.** 나눠 두니
 * `/admin` 과 `/admin/orders` 가 받는 파일에 tiptap 조각이 하나도 없다.
 *
 * **"눌러야 받는다" 는 아니다.** 이 화면은 폼이 늘 떠 있어서 열자마자 따라온다.
 * 눌러야 받게 만들 수도 있지만, 공지를 쓰러 온 사람에게 한 번 더 누르게 하는
 * 셈이다 — 이 화면의 일이 곧 글쓰기다.
 *
 * 나눠 두었는지는 무게 상한이 지킨다(bundle-budget-admin). 붙여 보니 이
 * 화면이 965KB 를 받아 상한(800KB)에 걸린다.
 *
