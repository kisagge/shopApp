...
export async function generateMetadata(): Promise<Metadata> {
  return { title: (await getT())('auth.resetHeading'), ...NO_INDEX };
}
  ...
  return (
    <div className="mx-auto flex w-full max-w-[420px] flex-col gap-8 px-4 py-16">
      <div className="flex flex-col gap-2">
        <h1 className="font-serif text-3xl font-medium tracking-tight">{t('auth.resetHeading')}</h1>
        <p className="text-[13px] text-[var(--fg-muted)]">{t('auth.resetLead')}</p>
      </div>
