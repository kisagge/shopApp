 ...
 * 상품 플레이스홀더 이미지 시드.
 *
 * 아무 데서나 사진을 긁어 오지 않는다. 남의 사진을 저장소에 넣어 두면
 * 나중에 출처를 설명할 수 없고, 포트폴리오로 공개할 때 그대로 문제가 된다.
 * 여기서 만드는 것은 PLAIN 팔레트의 톤 블록이라 **명백히 플레이스홀더**다.
 *
 * 실제 사진이 필요하면 fetch-product-photos.ts 를 쓴다 — 라이선스가 분명한
 * 곳(Unsplash)에서 받아 오고 작가를 함께 기록한다.
 *
