 ...
 *
 * ── 세 번째에 trace 를 열었다 (2026-09-14) ─────────────────────
 *
 * 문지기가 이번엔 실패 기록을 따로 남기게 돌려서 trace.zip 을 열 수 있었다. 이름 바꾸기(PATCH)가
 * **40.419 에 200 으로 끝났고**, 옛 주소 요청은 **13ms 뒤 40.432 에 시작해 넘김 없이 200** 을
 * 받았다(cache-control: no-store — 화면 캐시가 아니다). 즉 무효화가 끝난 뒤에도 **데이터 캐시에서
 * 옛 상품이 나왔다.** 혼자서는 안 나고 전체 실행에서만 나는 것도 들어맞는다 — 무효화 직전에 시작된
 * 다른 요청이 옛 값을 다시 써 넣는 3번 가설 쪽이다(재현은 여전히 못 했다).
 *
 * 그래서 타이밍을 맞추지 않고 **판단에서 캐시를 뺐다.** 상품 화면은 이제 옮겨진 주소인지를 캐시 없이
 * 먼저 묻고(getProductSlugMovedTo), 옮겨졌으면 캐시가 옛 상품을 주더라도 넘긴다. 구조가 되돌아가면
 * `test/slug-history.test.ts` 가 진다.
 *
 * ── 다음에 또 나오면 ──────────────────────────────────────────
 *
