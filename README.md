...
```
core 736 · web 2,013 · contract 209 · ui 103 · auth 38 · i18n 39 · db 28 · native 22 · mail 7
e2e 431 (Playwright, 33 파일)
```
...
**그래서 `pnpm ci:local` 이 문지기다.** 같은 순서로 같은 것을 돌리고 일회용
DB 까지 새로 만든다. **푸시 전에 반드시 돌린다** — 배포는 Vercel 이 Git 연동으로
직접 하므로, 이 워크플로를 꺼도 푸시하면 그대로 배포된다. 바꿔 말하면 지금은
저장소 쪽에서 막아 주는 것이 없다.
