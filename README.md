...
| 계층 | 위치 | 무엇을 지키는가 |
|---|---|---|
| **UI · 디자인** | `packages/ui/test`, `apps/web/test/*.test.tsx` | 컴포넌트 렌더링, axe 자동 접근성 검사, **디자인 토큰 명도대비 회귀** |
...
```
core 707 · web 1,827 · contract 208 · ui 101 · auth 38 · i18n 37 · db 28
e2e 262 (Playwright, 30 파일)
```
...
- **구매확정 뒤 반품** — 지금 상태 기계에서 구매확정은 종착이다. 실제 쇼핑몰은 확정
  뒤에도 하자 반품을 받으므로 상태 전이·정산·재고를 함께 고쳐야 한다.
- **푸시 알림** — APNs/FCM 인증서가 필요해서 뺐다.
- **시각 회귀 테스트** — 접근성과 마크업은 훑지만 픽셀은 보지 않는다.
